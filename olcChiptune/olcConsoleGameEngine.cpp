...
void olcConsoleGameEngine::DrawWireFrameModel(const std::vector<std::pair<float, float>>& vecModelCoordinates, float x, float y, float r, float s, short col, short c) {
...
void olcConsoleGameEngine::OnKeyUp(int key) {}

olcConsoleGameEngine::olcAudioSample::olcAudioSample() {

}

olcConsoleGameEngine::olcAudioSample::olcAudioSample(std::wstring sWavFile) {
...
unsigned int olcConsoleGameEngine::LoadAudioSample(std::wstring sWavFile) {
	if (!m_bEnableSound)
		return -1;
...
void olcConsoleGameEngine::PlaySample(int id, bool bLoop) {
...
void olcConsoleGameEngine::StopSample(int id) {

}

bool olcConsoleGameEngine::CreateAudio(unsigned int nSampleRate, unsigned int nChannels,
...
bool olcConsoleGameEngine::DestroyAudio() {
	m_bAudioThreadActive = false;
	return false;
}

void olcConsoleGameEngine::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1, DWORD dwParam2) {
...
void CALLBACK olcConsoleGameEngine::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
	((olcConsoleGameEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
}

void olcConsoleGameEngine::AudioThread() {
	m_fGlobalTime = 0.0;
...
float olcConsoleGameEngine::GetMixerOutput(int nChannel, double fGlobalTime, float fTimeStep) {
